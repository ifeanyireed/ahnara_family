...
export default function SeniorsWorkspaceLayout({ children }: { children: React.ReactNode }) {
  ...
  // Load seniors data and validate onboarding on mount
  useEffect(() => {
  ...
  const menuItems = [
    { name: "SafeDay", href: "/seniors", icon: IconLayoutDashboard, activeIcon: IconLayoutDashboardFilled },
    { name: "AI Companion", href: "/seniors/companion", icon: IconMessageChatbot, activeIcon: IconMessageChatbotFilled },
    { name: "Meds", href: "/seniors/medication", icon: IconPill, activeIcon: IconPillFilled },
    { name: "Vitals", href: "/seniors/vitals", icon: IconDeviceHeartMonitor, activeIcon: IconDeviceHeartMonitorFilled },
      ...
      <header className="px-8 py-5 flex flex-col md:flex-row items-center justify-between gap-4 bg-transparent border-none">
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className="px-5 h-11 rounded-full bg-red-600 border border-red-500/20 text-white font-black text-xs uppercase tracking-widest shadow-md shadow-red-600/10 hover:scale-105 active:scale-95 transition-all flex items-center gap-1.5"
          >
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-indigo-100 border border-indigo-200 flex items-center justify-center text-indigo-700 font-extrabold text-sm shadow-xs uppercase">
              {seniorsData.elderName.charAt(0)}
            </div>
            <div className="text-left hidden sm:block">
              <p className="font-bold text-sm text-slate-900 leading-none">{seniorsData.elderName}</p>
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1 block">
                Age: {new Date().getFullYear() - new Date(seniorsData.dob).getFullYear()} ({seniorsData.mobility})
              </span>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-red-500 text-xs font-black uppercase tracking-widest">
                  Elder Emergency Alert Dispatch
                </span>
              </div>
              <button
                ...
                <div className="flex flex-col items-center text-center gap-6">
                  ...
                  <div>
                    <h2 className="text-2xl font-black text-white text-display leading-tight">
                      Broadcasting Location &amp; Vitals
                    </h2>
                    <p className="text-slate-400 text-sm font-semibold mt-2 leading-relaxed">
                      Broadcasting elder emergency to nearest ambulances and alerting primary caregivers in the Trusted Circle...
                    </p>
                  </div>

                  {/* Vitals Summary Card */}
                  <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-left flex flex-col gap-2">
                    <span className="text-[10px] text-red-400 font-bold uppercase tracking-wider">Elder Profile Context</span>
                    <div className="grid grid-cols-2 gap-3 text-xs text-white">
                      <div>
                        <span className="text-slate-400">Name:</span>
                        <p className="font-bold mt-0.5">{seniorsData.elderName}</p>
                      </div>
                      <div>
                        <span className="text-slate-400">Mobility:</span>
                        <p className="font-bold mt-0.5 uppercase text-indigo-300">{seniorsData.mobility}</p>
                      </div>
                      <div>
                        <span className="text-slate-400">Diagnoses:</span>
                        <p className="font-bold mt-0.5 text-orange-400">
                          {Object.keys(seniorsData.risks).filter(k => seniorsData.risks[k]).join(", ") || "None"}
                        </p>
                      </div>
                      <div>
                        <span className="text-slate-400">Alert Priority:</span>
                        <p className="font-bold mt-0.5 uppercase text-red-500">Critical (Class A)</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-5 text-left">
                  ...
                  <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-4 flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-red-400 font-bold uppercase tracking-wider mt-0.5">
                        ETA: 9 Minutes (Lagos Triage Unit)
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="h-44 w-full bg-slate-800 rounded-2xl relative overflow-hidden border border-white/5 flex items-center justify-center">
                    ...
                    <div className="absolute bottom-3 right-3 bg-[#0D090C] border border-white/10 px-3 py-1 rounded-xl text-[10px] text-slate-400 font-bold">
                      Route: Dispatch Center C ➔ Elder GPS
                    </div>
                    <IconCurrentLocation className="w-8 h-8 text-indigo-400 animate-bounce" />
                  </div>
                  ...
                  <div className="bg-white/5 border border-white/10 p-4 rounded-2xl flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">Geriatric Critical Dispatch Team</p>
                    </div>
                    <a
                  ...
                  <div className="bg-white/5 border border-white/10 p-4 rounded-2xl flex items-start gap-3">
                    ...
                    <p className="text-xs text-slate-300 font-semibold leading-relaxed">
                      Remain calm. Caregivers linked to your Trusted Circle (NARA sync ID: {seniorsData.partnerCode}) have been paged and are receiving your coordinates.
                    </p>
                  </div>
